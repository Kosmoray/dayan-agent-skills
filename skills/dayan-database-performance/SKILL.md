---
name: dayan-database-performance
description: Diagnose query shape, boundedness, indexing, and transaction risks before tuning.
---
...
Diagnose query shape, boundedness, indexing, and transaction risks before tuning. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `database-performance-review.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Recommendations include query evidence and an explain-plan or measurement path.
...
```bash
python3 scripts/validate_public_skill.py dayan-database-performance
```
