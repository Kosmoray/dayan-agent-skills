---
name: dayan-system-ops
description: Perform read-only system triage and translate machine symptoms into safe next actions.
---
...
Perform read-only system triage and translate machine symptoms into safe next actions. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `system-triage.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Observations are separated from hypotheses and any state-changing command.
...
```bash
python3 scripts/validate_public_skill.py dayan-system-ops
```
