---
name: dayan-crap-analysis
description: Combine complexity and test coverage evidence to rank the code most likely to fail during change.
---
...
Combine complexity and test coverage evidence to rank the code most likely to fail during change. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `risk-ranking.json` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Scores preserve raw complexity, coverage, formula, and uncertainty.
...
```bash
python3 scripts/validate_public_skill.py dayan-crap-analysis
```
