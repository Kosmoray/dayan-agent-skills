---
name: dayan-brand-guidelines
description: Translate brand identity, visual rules, voice, and claims into an executable public guide.
---

# Dayan Brand Guidelines

## Use when

Translate brand identity, visual rules, voice, and claims into an executable public guide. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `brand-guidelines.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Every rule includes a usable example, anti-example, and evidence boundary.
...
```bash
python3 scripts/validate_public_skill.py dayan-brand-guidelines
```
