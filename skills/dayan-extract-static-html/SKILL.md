---
name: dayan-extract-static-html
description: Freeze a specific webpage state into an offline review artifact without claiming full application behavior.
---

# Dayan Extract Static Html

## Use when

Freeze a specific webpage state into an offline review artifact without claiming full application behavior. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `snapshot.html` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Assets, visible state, viewport, source date, and missing behavior are disclosed.
...
```bash
python3 scripts/validate_public_skill.py dayan-extract-static-html
```
