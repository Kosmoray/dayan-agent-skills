---
name: dayan-local-tools
description: Pin repository-scoped developer tools so setup and CI use the same versions.
---
...
Pin repository-scoped developer tools so setup and CI use the same versions. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `tool-manifest.json` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Tool versions, restore command, invocation, and removal path are explicit.
...
```bash
python3 scripts/validate_public_skill.py dayan-local-tools
```
