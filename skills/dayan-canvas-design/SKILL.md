---
name: dayan-canvas-design
description: Turn a theme into a focused high-resolution visual composition with one visual philosophy.
---
...
Turn a theme into a focused high-resolution visual composition with one visual philosophy. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `canvas-artifact.png` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Composition, hierarchy, typography, contrast, and target resolution are reviewed.
...
```bash
python3 scripts/validate_public_skill.py dayan-canvas-design
```
