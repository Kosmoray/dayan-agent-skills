---
name: dayan-dispatch
description: Split a bounded plan into independent assignments with non-overlapping write scopes.
---
...
Split a bounded plan into independent assignments with non-overlapping write scopes. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `dispatch-map.json` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Dependencies are acyclic and concurrent assignments do not share write targets.
...
```bash
python3 scripts/validate_public_skill.py dayan-dispatch
```
