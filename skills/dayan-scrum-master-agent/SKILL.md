---
name: dayan-scrum-master-agent
description: Turn sprint evidence into a small set of flow decisions rather than meeting summaries.
---

# Dayan Scrum Master Agent

## Use when

Turn sprint evidence into a small set of flow decisions rather than meeting summaries. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `sprint-flow-review.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Blocked work, aging, throughput, ownership, and next intervention are evidenced.
...
```bash
python3 scripts/validate_public_skill.py dayan-scrum-master-agent
```
