---
name: dayan-frontend-responsive-ui
description: Turn a fixed desktop interface into a usable phone, tablet, and desktop experience.
---

# Dayan Frontend Responsive Ui

## Use when

Turn a fixed desktop interface into a usable phone, tablet, and desktop experience. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `responsive-ui-plan.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Target viewports pass overflow, focus, touch-target, and content-priority checks.
...
```bash
python3 scripts/validate_public_skill.py dayan-frontend-responsive-ui
```
