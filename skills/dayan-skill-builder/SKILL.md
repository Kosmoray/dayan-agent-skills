---
name: dayan-skill-builder
description: Forge a recurring workflow into a triggerable Skill with progressive disclosure and verification.
---
...
Forge a recurring workflow into a triggerable Skill with progressive disclosure and verification. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `skill-package.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Description, trigger boundary, workflow, fixtures, provenance, and validation all pass.
...
```bash
python3 scripts/validate_public_skill.py dayan-skill-builder
```
