---
name: dayan-aws-solution-architect
description: Translate workload, security, operations, team capability, and cost constraints into a reviewable AWS architecture.
---

# Dayan Aws Solution Architect

## Use when

Translate workload, security, operations, team capability, and cost constraints into a reviewable AWS architecture. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `aws-architecture-decision.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Assumptions, alternatives, failure domains, cost drivers, security, and rollback are explicit.
...
```bash
python3 scripts/validate_public_skill.py dayan-aws-solution-architect
```
