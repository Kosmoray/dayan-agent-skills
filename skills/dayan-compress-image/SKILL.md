---
name: dayan-compress-image
description: Reduce image size while preserving the dimensions, transparency, and visual quality required by delivery.
---

# Dayan Compress Image

## Use when

Reduce image size while preserving the dimensions, transparency, and visual quality required by delivery. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `compressed-image` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Before and after size, dimensions, format, and visual inspection are recorded.
...
```bash
python3 scripts/validate_public_skill.py dayan-compress-image
```
