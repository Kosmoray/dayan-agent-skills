---
name: dayan-write-concisely
description: Reduce human-facing writing to the shortest form that preserves decision, evidence, and action.
---

# Dayan Write Concisely

## Use when

Reduce human-facing writing to the shortest form that preserves decision, evidence, and action. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `concise-draft.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. The reader can identify conclusion, reason, action, and uncertainty in one pass.
...
```bash
python3 scripts/validate_public_skill.py dayan-write-concisely
```
