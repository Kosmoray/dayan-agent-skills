---
name: dayan-code-reviewer
description: Review a concrete code change for correctness, data safety, runtime failure, and maintainability.
---
...
Review a concrete code change for correctness, data safety, runtime failure, and maintainability. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `code-review.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Every blocking finding cites a repository-relative path, trigger, consequence, and smallest fix.
...
```bash
python3 scripts/validate_public_skill.py dayan-code-reviewer
```
