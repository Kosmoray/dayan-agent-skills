---
name: dayan-theme-factory
description: Compile reusable color, typography, spacing, component, and motion tokens for a coherent artifact family.
---
...
Compile reusable color, typography, spacing, component, and motion tokens for a coherent artifact family. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `theme-package.json` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Tokens have semantic names, contrast checks, component examples, and reduced-motion rules.
...
```bash
python3 scripts/validate_public_skill.py dayan-theme-factory
```
