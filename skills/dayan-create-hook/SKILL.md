---
name: dayan-create-hook
description: Create one minimal lifecycle guard for a repeated deterministic rule.
---
...
Create one minimal lifecycle guard for a repeated deterministic rule. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `hook-package.json` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Trigger, matcher, timeout, failure policy, log, and rollback are testable.
...
```bash
python3 scripts/validate_public_skill.py dayan-create-hook
```
