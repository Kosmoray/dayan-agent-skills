---
name: dayan-a11y-audit
description: Audit a digital interface for accessibility barriers and translate findings into reproducible fixes.
---
...
Audit a digital interface for accessibility barriers and translate findings into reproducible fixes. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `accessibility-audit.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Every finding names the affected user, criterion, evidence, and retest.
...
```bash
python3 scripts/validate_public_skill.py dayan-a11y-audit
```
