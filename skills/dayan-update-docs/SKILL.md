---
name: dayan-update-docs
description: Update only the documentation whose user-visible contract changed.
---
...
Update only the documentation whose user-visible contract changed. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `documentation-change.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Every edit maps to a real code or behavior change and stale claims are removed.
...
```bash
python3 scripts/validate_public_skill.py dayan-update-docs
```
