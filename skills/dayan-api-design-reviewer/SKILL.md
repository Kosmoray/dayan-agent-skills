---
name: dayan-api-design-reviewer
description: Review an API change as a durable contract before implementation or release.
---
...
Review an API change as a durable contract before implementation or release. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `api-review.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Examples cover success, malformed input, authorization, pagination, and versioning.
...
```bash
python3 scripts/validate_public_skill.py dayan-api-design-reviewer
```
