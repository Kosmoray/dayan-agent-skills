---
name: dayan-cover-image
description: Convert an article or campaign idea into a legible, distinctive cover image direction.
---
...
Convert an article or campaign idea into a legible, distinctive cover image direction. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `cover-brief.json` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Thumbnail legibility, title hierarchy, rights, crop, and channel dimensions pass.
...
```bash
python3 scripts/validate_public_skill.py dayan-cover-image
```
