---
name: dayan-second-opinion
description: Challenge an important judgment with an independent counter-case and explicit decision impact.
---

# Dayan Second Opinion

## Use when

Challenge an important judgment with an independent counter-case and explicit decision impact. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `second-opinion.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. The review names the strongest alternative, disconfirming evidence, and decision delta.
...
```bash
python3 scripts/validate_public_skill.py dayan-second-opinion
```
