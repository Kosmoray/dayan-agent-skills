---
name: dayan-code-to-design
description: Extract a reviewable design system and component inventory from a running frontend.
---

# Dayan Code To Design

## Use when

Extract a reviewable design system and component inventory from a running frontend. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `design-system.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Tokens and components are traced to source selectors or files.
...
```bash
python3 scripts/validate_public_skill.py dayan-code-to-design
```
