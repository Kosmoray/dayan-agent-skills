---
name: dayan-industry-research
description: Build an evidence-led industry map from primary sources without turning market narrative into fact.
---

# Dayan Industry Research

## Use when

Build an evidence-led industry map from primary sources without turning market narrative into fact. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `industry-map.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Claims record source, date, geography, scope, and confidence.
...
```bash
python3 scripts/validate_public_skill.py dayan-industry-research
```
