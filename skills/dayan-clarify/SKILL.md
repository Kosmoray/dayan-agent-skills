---
name: dayan-clarify
description: Separate discoverable facts from preferences and ask only questions that can change the result.
---
...
Separate discoverable facts from preferences and ask only questions that can change the result. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `clarification-brief.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Each question states which decision it can change.
...
```bash
python3 scripts/validate_public_skill.py dayan-clarify
```
