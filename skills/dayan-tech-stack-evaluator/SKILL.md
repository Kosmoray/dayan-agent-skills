---
name: dayan-tech-stack-evaluator
description: Compare technology choices against product constraints, team capability, lock-in, cost, and failure recovery.
---

# Dayan Tech Stack Evaluator

## Use when

Compare technology choices against product constraints, team capability, lock-in, cost, and failure recovery. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `stack-decision.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Criteria are weighted before scoring and sensitivity to changed assumptions is shown.
...
```bash
python3 scripts/validate_public_skill.py dayan-tech-stack-evaluator
```
