---
name: dayan-marketplace-publishing
description: Package a reusable Skill or agent so strangers can install, understand, validate, and remove it.
---

# Dayan Marketplace Publishing

## Use when

Package a reusable Skill or agent so strangers can install, understand, validate, and remove it. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `release-package.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Manifest, paths, provenance, redline scan, install smoke, and version agree.
...
```bash
python3 scripts/validate_public_skill.py dayan-marketplace-publishing
```
