---
name: dayan-explain
description: Explain a complex system so a decision-maker and operator can each repeat the relevant part.
---
...
Explain a complex system so a decision-maker and operator can each repeat the relevant part. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `system-explainer.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. The output separates executive decision, operator action, evidence, and unknowns.
...
```bash
python3 scripts/validate_public_skill.py dayan-explain
```
