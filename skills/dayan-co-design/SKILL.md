---
name: dayan-co-design
description: Coordinate product, visual, frontend, and verification work without mixing ownership.
---
...
Coordinate product, visual, frontend, and verification work without mixing ownership. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `co-design-contract.json` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Each workstream has inputs, outputs, owner, dependencies, and acceptance evidence.
...
```bash
python3 scripts/validate_public_skill.py dayan-co-design
```
