---
name: dayan-llm-council
description: Compare independent reasoning paths for a consequential decision and expose disagreement before synthesis.
---
...
Compare independent reasoning paths for a consequential decision and expose disagreement before synthesis. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `council-decision.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. Independent positions are preserved before the final synthesis.
...
```bash
python3 scripts/validate_public_skill.py dayan-llm-council
```
