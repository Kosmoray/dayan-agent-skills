---
name: dayan-humanize
description: Remove formulaic model language while preserving facts, author intent, and audience action.
---
...
Remove formulaic model language while preserving facts, author intent, and audience action. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `humanized-draft.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. No fact is added, author voice remains consistent, and repeated explanation is removed.
...
```bash
python3 scripts/validate_public_skill.py dayan-humanize
```
