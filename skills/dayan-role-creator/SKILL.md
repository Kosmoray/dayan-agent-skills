---
name: dayan-role-creator
description: Define a reusable agent role with one responsibility, bounded authority, and an evaluation contract.
---
...
Define a reusable agent role with one responsibility, bounded authority, and an evaluation contract. Use it only when the requested result matches this scope; a narrower factual question or one-off edit should stay narrow.
...
3. **Produce:** create `role-contract.md` with repository-relative or public references and no private machine dependencies.
4. **Verify:** run the checks in the starter contract. The role has explicit inputs, outputs, tools, exclusions, and takeover conditions.
...
```bash
python3 scripts/validate_public_skill.py dayan-role-creator
```
