...
| False-positive control | Treat fluent output, pretty UI, and passing structure checks as insufficient evidence | `dayan-adversarial-reviewer` | failure gallery where attractive artifacts are correctly rejected |
...
| Tool-before-agent | Turn deterministic checks into scripts before asking an agent to remember them | `dayan-hook-factory` | hook fixtures and reproducible failure cases |
| Human authority ledger | Name the actions that remain human-controlled before a workflow starts | `dayan-agent-designer` | agent contracts with authority and escalation examples |
...
The current method-card set is in [`docs/methods.md`](methods.md). It covers complexity, decomposition, adversarial validation, evidence chains, reversible action, falsifiable framing, human authority, completion levels, project harnesses, delivery assets, simple communication, and audience-clean artifacts.

## What not to publish

- personal memory;
- customer-specific material;
- credentials, private paths, private infrastructure, or account workflows;
- claims about revenue, adoption, ranking, or compatibility without public evidence;
- internal names that do not help an outside user apply the method.
